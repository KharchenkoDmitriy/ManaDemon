...
3. **for anything the engine or the recorder touches, both harnesses still pass**:
   ...
   assertions), `tools/run.sh tools/replaycheck.lua` (the trace and the replay state machine, 27
   assertions), `tools/run.sh tools/replayui.lua` (the replay window painted under the stub, 33
